...
#### � 構建架構說明
*   **統一構建**：使用 `npm run tauri build` 命令一次性構建前端（Vite）和後端（Rust）。
*   **多階段構建**：
    1.  **Tauri Builder Stage**: 安裝 Node.js 20 + Rust 1.85，執行完整 Tauri 構建流程
    2.  **Runtime Stage**: 僅包含運行時依賴，大幅減小最終鏡像體積
*   **構建產物**：
    - 前端資源：`/app/dist` (由 Vite 構建)
    - 後端二進制：`/app/src-tauri/target/release/antigravity_tools`
